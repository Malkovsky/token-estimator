...
Measure minimal, expected, and full context across skills, instructions, prompts, agents, and MCP tool definitions in a public GitHub repository.

Paste a repository or folder URL to inventory its skills, instructions, rules, prompts, agents, and MCP configuration. The report separates a lightweight lower bound, a practical expected-use profile, and a reasonable static upper bound.

## What you can do

- Analyze an entire public GitHub repository or one of its folders.
- Compare minimal, expected, and full totals.
...
- minimal tokens across artifact names and descriptions;
- expected tokens for the content normally needed to use each artifact;
- full tokens including reasonable optional supporting content;
...
Minimal is the compact identity a harness can use to decide whether an artifact is relevant. Expected is a transparent practical default, not a guarantee of any particular harness implementation. Full is the union of recognized static content that could reasonably be loaded over time, not a claim that every artifact is loaded simultaneously. Runtime MCP connection configuration is excluded.

| Artifact | Minimal | Expected | Full |
| --- | --- | --- | --- |
| Skill | Name and description | Minimal plus `SKILL.md` instructions | Expected plus recognized supporting files |
| MCP tool | Name and description | Minimal plus input schema | Expected plus output schema, title, and standard annotations |
| Agent | Name and description when declared | Complete agent definition | Same as expected |
| Rule or prompt | Name and description when declared | Complete file content | Same as expected |
...
[![Token summary](https://how-much-tokens.onrender.com/badge/github/Malkovsky/token-estimator.svg?metric=summary)](https://how-much-tokens.onrender.com/github/Malkovsky/token-estimator/latest)

Repository reports show a combined `Min | value | Avg | value | All | value` summary plus separate minimal and full-token badges, with a copy button next to each. `Min`, `Avg`, and `All` correspond to the Minimal, Expected, and Full profiles. Paste the resulting Markdown into a README. The existing `metric=metadata` and `metric=total` query values remain stable for compatibility:

```markdown
[![Token summary](https://how-much-tokens.onrender.com/badge/github/OWNER/REPOSITORY.svg?metric=summary)](https://how-much-tokens.onrender.com/github/OWNER/REPOSITORY/latest)
[![Minimal tokens](https://how-much-tokens.onrender.com/badge/github/OWNER/REPOSITORY.svg?metric=metadata)](https://how-much-tokens.onrender.com/github/OWNER/REPOSITORY/latest)
[![Full tokens](https://how-much-tokens.onrender.com/badge/github/OWNER/REPOSITORY.svg?metric=total)](https://how-much-tokens.onrender.com/github/OWNER/REPOSITORY/latest)
```

The **Copy** action preserves an explicitly supplied branch, tag, or commit. When the original repository URL and advanced options contain no ref, it follows the repository's default branch. Its `/github/OWNER/REPOSITORY/latest` link resolves the same ref to an immutable report. Existing commit-pinned badge URLs remain supported.

Badge URLs follow the repository's default branch unless `ref` is supplied. To follow a specific branch or tag, use the same `ref` on both the badge and latest-report link—for example, `REPOSITORY.svg?metric=summary&ref=release%2F2.x` and `/latest?ref=release%2F2.x`. Badge and latest-report URLs also accept the same optional `path` and `encoding` query parameters as repository analysis. Each moving badge periodically resolves its branch head; branches at the same commit share one cached token summary.
...
- `GET /github/{owner}/{repository}/latest` — resolve the default branch (or optional `ref`) and redirect to its immutable report.
...
Repository reports treat each tool's name and description as minimal context. Expected is a canonical definition containing the name, description, and input schema. Full additionally includes the title, output schema, and standard MCP annotations when present. Examples and descriptions nested inside those schemas are included. Icons, `_meta`, arbitrary extension fields, runtime results, and server connection details remain excluded.

Each MCP tool card shows minimal, expected, optional context, and full. Expected and full are tokenized as complete canonical JSON definitions, so their field names and serialization punctuation are included. The displayed profiles and components explain the accounting but are not additive. When a supported MCP SDK dependency is detected but no valid snapshot exists, the report warns that the definitions cannot be estimated because the service does not execute the server or call `tools/list`.
